...
class HuffmanView(BaseSubPage):
    ...
    def add_node(self, node_id, value, char, is_leaf, pos):
        if node_id in self.node_items:
            return self.node_items[node_id]

        node_item = HTreeNodeItem(node_id, value, char, is_leaf)
...
class HTreeNodeItem(QGraphicsObject):
    def __init__(self, node_id, value, char, is_leaf, parent=None):
    ...
    def paint(self, painter: QPainter, option, widget=None):
        ...
        brush = QBrush(QColor(COLORS["norm"]))
        painter.setPen(pen)
        ...
        if self.is_leaf:
            ...
            painter.drawText(self.boundingRect(), Qt.AlignmentFlag.AlignCenter, self.char)
            painter.setPen(Qt.GlobalColor.darkGray)
            painter.setFont(QFont("Consolas", 9, QFont.Weight.ExtraBold))
            painter.drawText(-15, 15, self.value)
